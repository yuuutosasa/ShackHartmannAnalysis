...
def compute_shifts(img, grid_size=16, pad_top=8, pad_left=8):
    H, W = img.shape
    ...
    for i in range(N_long):
        for j in range(N_lat):
            shifts[i, j] = center_of_mass(
                img[
                    pad_top + grid_size * i : pad_top + grid_size - 1 + grid_size * i,
